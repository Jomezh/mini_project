...
class PairingScreen(Screen):
    ...
    def build_ui(self):
        ...
        self.title_label = Label(
            text='MiniK', font_size='18sp', bold=True, size_hint=(1, 0.08)
        )
        self.status_label = Label(
            text='Open MiniK app on your phone & scan this QR code to pair',
            font_size='11sp', size_hint=(1, 0.12),
            halign='center', valign='middle', color=(0.8, 0.8, 0.8, 1)
        )
        self.status_label.bind(size=self.status_label.setter('text_size'))

        self.qr_image = Image(
            size_hint=(1, 1), allow_stretch=True, keep_ratio=True, opacity=1
        )
        self.device_id_label = Label(
            text='', font_size='9sp', size_hint=(1, 0.06), color=(0.5, 0.5, 0.5, 1)
        )
        self.action_btn = Button(
            text='Look for Phone',
            background_color=(0.15, 0.55, 0.15, 1),
            font_size='10sp', size_hint=(1, 0.25), halign='center'
        )
        self.action_btn.bind(on_press=self.on_action_btn)

        btn_row = BoxLayout(orientation='horizontal', size_hint=(1, 0.25), spacing=5)
        ...
        self.pair_btn.bind(
            on_press=lambda x: self.controller.start_pairing() if self.controller else None
        )
        self.shutdown_btn = Button(
            text='Power Off',
            background_color=(0.55, 0.1, 0.1, 1), font_size='10sp'
        )
        self.shutdown_btn.bind(
            on_press=lambda x: self.controller.shutdown_device() if self.controller else None
        )
    ...
    def on_action_btn(self, *args):
        if not self.controller:
            return
        if self.action_btn_mode == 'scan':
            self.controller.rescan_for_devices()
        elif self.action_btn_mode == 'cancel':
            self.controller.stop_ble_pairing()
        elif self.action_btn_mode == 'retry':
    ...
    def show_qr(self, message: str = None):
        ...
        self.action_btn.text     = 'Look for Phone'
    ...
    def tick_hotspot_prompt(self, dt):
    ...
    def update_hotspot_label(self):
        s = self.retry_countdown
        r = self.hotspot_retries_left
        self.status_label.text = (
            f'Enable hotspot on {self.hotspot_device}\n'
            f'Retrying in {s}s — '
